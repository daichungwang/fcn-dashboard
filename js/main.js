...
const appState = {
  ...
  currentGroup: "all", // all / 核心 / 防禦 / 成長 / 收益 / 投機
  currentScope: "all", // all / recommended
  newsExpanded: {
    international: false,
    finance: false,
    ai: false,
...
function deltaClass(v) {
  return safe(v) >= 0 ? "delta-positive" : "delta-negative";
}
...
function getBaselineScore(raw) {
  if (raw.baseline_score != null) return safe(raw.baseline_score);
...
function calcAdjustmentModel(raw, baselineGroup) {
  ...
  const weightedScore = score1m * 0.1 + score6m * 0.3 + score12m * 0.6;
  const factor = getVolFactor(baselineGroup);
  const adjustment = weightedScore * factor;
...
function calcNewsWeight(daysAgo, importance = 1) {
...
function calcStockEventFromNews(newsList = []) {
  ...
  newsList.forEach((n) => {
    const weight = calcNewsWeight(n.daysAgo, n.importance || 1);
...
function calcEventImpact(raw) {
  const stockEvent = calcStockEventFromNews(raw.newsItems || []);
  ...
  return Number(clamp(stockEvent + sectorEvent + riskPreferenceEvent + macroEvent + eventBias, -3, 3).toFixed(2));
}
...
function getPRiskScore(strike, ki) {
  if (strike == null || ki == null) return null;
...
function calcSRIFromBasket(basket) {
  if (!Array.isArray(basket) || basket.length === 0) return 0;
  const worstAvg = basket.reduce((sum, s) => sum + getWorstOfPenalty(s.baseline_group), 0) / basket.length;
  const assyAvg = basket.reduce((sum, s) => sum + getAssyPenalty(s.baseline_group), 0) / basket.length;
  return Number((worstAvg * 0.6 + assyAvg * 0.4).toFixed(2));
}

function getEkiScore(eki) {
  return eki ? 2 : 0;
}
...
async function loadTemplates() {
  try {
    const res = await fetch("./data/fcn_templates.json");
    if (!res.ok) {
      appState.templates = [];
      return;
    }
    const data = await res.json();
    appState.templates = Array.isArray(data) ? data : [];
  } catch {
    appState.templates = [];
  }
}
...
function basketAllowed(basket, size) {
  if (!Array.isArray(basket) || basket.length !== size) return false;
...
function avg(arr) {
  ...
  return arr.reduce((sum, v) => sum + safe(v), 0) / arr.length;
}

function scoreBasket(basket, tpl) {
  const avgPure = avg(basket.map((s) => s.pure_score));
  const avgEvent = avg(basket.map((s) => s.event_score));
  const avgVol = avg(basket.map((s) => s.vol_score));
  const sri = calcSRIFromBasket(basket);

  const purePack = calcFCNPure(tpl, avgPure, sri);
  const eventPack = calcFCNEvent(tpl, avgPure, avgEvent, avgVol);

  if (purePack.fcn_pure == null) return null;

  const finalScore = Number((0.7 * purePack.fcn_pure + 0.3 * eventPack.fcn_event).toFixed(2));
...
function overlapRatio(a, b) {
  ...
  const inter = b.filter((x) => setA.has(x.symbol)).length;
...
function deduplicateBaskets(baskets) {
  const result = [];
  baskets.forEach((b) => {
    const exists = result.find((r) => overlapRatio(r.basket, b.basket) > 0.75 && r.template.id === b.template.id);
...
function pickTopByCategory(baskets) {
  ...
  baskets.forEach((b) => {
    const tag = b.template.risk_tag || "other";
    if (!groups[tag]) groups[tag] = [];
    groups[tag].push(b);
  });

  return [
    ...groups.conservative.slice(0, 2),
    ...groups.normal.slice(0, 3),
    ...groups.aggressive.slice(0, 2),
    ...groups.dream.slice(0, 1),
    ...groups.take_stock.slice(0, 1),
    ...groups.other.slice(0, 2),
  ];
}

function generateBasketResults() {
  if (!appState.templates.length || !appState.poolData.length) {
    appState.basketResults = [];
    return;
  }

  const candidates = appState.poolData.filter((s) => s.baseline_group !== "投機");

  const all = [];

  appState.templates.forEach((tpl) => {
    const k = safe(tpl.basket_size, 0);
    if (!k || k > candidates.length) return;

    const combos = combinations(candidates, k);
    combos.forEach((basket) => {
      if (!basketAllowed(basket, k)) return;
      const scored = scoreBasket(basket, tpl);
      if (scored) all.push(scored);
    });
  });

  all.sort((a, b) => b.finalScore - a.finalScore);
  appState.basketResults = pickTopByCategory(deduplicateBaskets(all));
}
...
async function loadPoolData() {
  try {
    ...
    if (!Array.isArray(raw)) throw new Error("pool.json 不是陣列格式");

    const firstPass = raw.map((item) => {
      const baseline_group = getBaselineGroup(item);
      const baseline_score = getBaselineScore(item);
      const volData = calcVolFromReturns(item);
      const adjustmentModel = calcAdjustmentModel(item, baseline_group);
      const pure_score = Number((baseline_score + adjustmentModel.adjustment).toFixed(2));
      const eventImpact = calcEventImpact(item);
      const event_score = Number((pure_score + eventImpact * 0.3).toFixed(2));
      ...
      return {
        ...
        eventImpact,
        event_score,
        pure_label: groupFromScore(pure_score),
        event_label: groupFromScore(event_score),
        dPure: Number((pure_score - baseline_score).toFixed(2)),
        dEvent: Number((event_score - pure_score).toFixed(2)),
      };
    });

    const sriAll = calcSRIFromBasket(firstPass);

    appState.poolData = firstPass.map((stock) => {
      const purePack = calcFCNPure(stock, stock.pure_score, sriAll);
      const eventPack = calcFCNEvent(stock, stock.pure_score, stock.event_score, stock.vol_score);
      return {
        ...stock,
        ...purePack,
        ...eventPack,
    ...
    if (m3a) m3a.innerHTML = `<div class="error-box">載入 data/pool.json 失敗：${err.message}</div>`;

    const m3b = document.getElementById("m3b-content");
    if (m3b) m3b.innerHTML = `<div class="error-box">Basket 生成失敗：${err.message}</div>`;
  }
}
...
function injectUtilityStyles() {
  ...
  style.textContent = `
    ...
    .summary-card,.stock-card,.position-card,.news-card,.model-card,.basket-card{
      background:#fff;border:1px solid #e5e7eb;border-radius:18px;box-shadow:0 2px 10px rgba(0,0,0,.05);
    ...
    .news-table{width:100%;border-collapse:collapse;border-radius:14px;overflow:hidden;font-size:14px;margin-top:12px;}
    .news-table th,.news-table td{border:1px solid #d1d5db;padding:12px 10px;text-align:left;background:#fff;vertical-align:top;}
    .news-table th{background:#6b6b6b;color:#fff;font-size:15px;}
    ...
    .m3-outline{background:#f9fafb;border:1px solid #e5e7eb;border-radius:14px;padding:14px;margin-bottom:14px;}
    .error-box{background:#fff7ed;border:1px solid #fdba74;color:#9a3412;border-radius:14px;padding:14px;margin-bottom:14px;}
    .card-collapse-body{display:none;margin-top:14px;padding-top:8px;border-top:1px solid #e5e7eb;}
    .basket-tags{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0;}
    .basket-tag{padding:6px 10px;border-radius:999px;font-size:12px;border:1px solid #d1d5db;background:#f9fafb;}
  ...
  container.innerHTML = `
    ...
    <div class="model-card">
      ...
      <table class="news-table">
        <thead>
          <tr>
            <th>指標</th><th>數值</th><th>前值</th><th>變動</th><th>解讀</th>
          </tr>
        </thead>
        <tbody>
          ${marketRows.map((r) => `
            <tr>
              <td>${r.name}</td><td>${r.value}</td><td>${r.prev}</td>
              <td class="delta-negative">${r.change}</td><td>${r.note}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
  ...
  container.innerHTML = `
    <div class="model-card">
      ...
      <div class="summary-grid">
        <div class="summary-card"><div class="summary-title">全部</div><div class="summary-value">${total}</div></div>
        <div class="summary-card"><div class="summary-title">健康</div><div class="summary-value up">${healthy}</div><div class="mini-note">${Math.round((healthy / total) * 100)}%</div></div>
        <div class="summary-card"><div class="summary-title">追蹤</div><div class="summary-value warn">${watch}</div><div class="mini-note">${Math.round((watch / total) * 100)}%</div></div>
        <div class="summary-card"><div class="summary-title">風險 / 待確認</div><div class="summary-value down">${risk} / 0</div></div>
      </div>
      ...
      <div id="positionsContainer">
        ${positions.map((p) => `
          <div class="position-card m2-position-card">
            ...
            <div class="line"><strong>距離下限價：</strong> --</div>
            <button class="btn m2-detail-btn" onclick="toggleM2Card(this)">展開整組</button>
            <div class="card-collapse-body m2-card-body">
...
function renderDashboard() {
  ...
  const recommended = rows.filter((s) => s.fcn_pure != null);
  const pureUps = rows.filter((s) => s.dPure > 0).length;
  const avgBasketPure = appState.basketResults.length
    ? avg(appState.basketResults.map((b) => b.fcn_pure))
    : 0;

  if (m1Stock) m1Stock.textContent = `股票建議率：${Math.round((rows.filter((s) => s.baseline_group !== "投機").length / rows.length) * 100)}%`;
  if (m1Fcn) m1Fcn.textContent = `FCN 建議率：${Math.round((recommended.length / rows.length) * 100)}%`;
  if (m2Risk) m2Risk.textContent = `風險指數：${(10 - (pureUps / rows.length) * 5).toFixed(1)}`;
  if (m2DPure) m2DPure.textContent = `ΔPure：${pureUps}`;
  if (m2DEvent) m2DEvent.textContent = appState.newsMode === "news" ? `ΔEvent：${rows.filter((s) => s.dEvent > 0).length}` : "ΔEvent：--";
  if (m3Score) m3Score.textContent = `適合度：${avgBasketPure.toFixed(2)}`;
...
function renderM3Model() {
  ...
  el.innerHTML = `
    <div class="model-card">
      ...
      <div style="display:${appState.m3ModelExpanded ? "block" : "none"};">
        <div class="m3-outline">
          ...
          <div class="line">FCN Pure Score = 0.4×股票 Pure + 0.2×利率 + 0.1×天期 + 0.1×P Risk + 0.1×SRI + EKI</div>
          <div class="line">FCN Event Score = 0.5×股票 Pure + 0.25×Event + 0.25×Vol + EKI</div>
        </div>
      </div>
    </div>
  `;
...
function renderM3Summary() {
  ...
  const allow = rows.filter((s) => s.fcn_pure != null).length;

  let upTitle = "Event 上修";
  let downTitle = "Event 下修";
  let upCount = rows.filter((s) => s.dEvent > 0).length;
  let downCount = rows.filter((s) => s.dEvent < 0).length;

  if (appState.newsMode === "pure") {
    upTitle = "Pure 上修";
    downTitle = "Pure 下修";
    upCount = rows.filter((s) => s.dPure > 0).length;
    downCount = rows.filter((s) => s.dPure < 0).length;
  }

  el.innerHTML = `
    <div class="summary-grid">
      <div class="summary-card"><div class="summary-title">股票總數</div><div class="summary-value">${total}</div></div>
      <div class="summary-card"><div class="summary-title">可做 FCN</div><div class="summary-value">${allow}</div></div>
      <div class="summary-card"><div class="summary-title">${upTitle}</div><div class="summary-value up">${upCount}</div></div>
      <div class="summary-card"><div class="summary-title">${downTitle}</div><div class="summary-value down">${downCount}</div></div>
    </div>
...
function renderStockCard(stock) {
  ...
  return `
    <div class="stock-card m3a-stock-card">
      ...
      <div class="stock-meta">${stock.sector || stock.ai_classification || "--"} ｜ <span class="${labelClass(stock.baseline_group)}">${stock.baseline_group}</span></div>
      ...
      <div class="btn-row" style="margin-top:12px;">
        <button class="btn m3a-detail-btn" onclick="toggleM3Card(this)">展開整卡</button>
      </div>

      <div class="card-collapse-body m3-card-body">
...
function renderM3A() {
  ...
  let rows = [...appState.poolData];

  if (appState.currentGroup !== "all") {
    rows = rows.filter((s) => s.baseline_group === appState.currentGroup);
  }
  if (appState.currentScope === "recommended") {
    rows = rows.filter((s) => s.fcn_pure != null);
  }

  container.innerHTML = rows.length
    ? rows.map(renderStockCard).join("")
    : `<div class="model-card">目前沒有資料</div>`;
}

// ---------- M3B ----------
function renderBasketCard(b, index) {
  return `
    <div class="basket-card m3b-card">
      <div class="basket-head">Top ${index + 1}｜${b.template.name}</div>
      <div class="line"><strong>摘要：</strong>${b.basket.map((s) => s.symbol).join(" / ")}</div>
      <div class="line"><strong>Final：</strong>${b.finalScore}</div>
      <button class="btn m3b-detail-btn" onclick="toggleM3BCard(this)">展開整組</button>

      <div class="card-collapse-body m3b-card-body">
        <div class="basket-tags">
          ${b.basket.map((s) => `<span class="basket-tag ${labelClass(s.baseline_group)}">${s.symbol}｜${s.baseline_group}</span>`).join("")}
        </div>
        ...
        <div class="line"><strong>利率 / 天期：</strong>${b.template.coupon}% / ${b.template.tenor} 月</div>
        <div class="line"><strong>Strike / KI：</strong>${b.template.strike} / ${b.template.ki}</div>
...
function renderM3B() {
  ...
  if (!appState.basketResults.length) {
  ...
  el.innerHTML = `
    <div class="model-card">
      <div class="btn-row">
        <button class="btn" onclick="expandAllM3B()">全部展開</button>
        <button class="btn" onclick="collapseAllM3B()">全部收合</button>
      </div>
      ${appState.basketResults.map(renderBasketCard).join("")}
    </div>
  `;
}
...
function renderStockSearchResult(query = "") {
  ...
  const hit = appState.poolData.find(
    (s) => s.symbol.toUpperCase() === q || String(s.name || "").toUpperCase().includes(q)
  );
  ...
  el.innerHTML = renderStockCard(hit);
}
...
window.toggleM2Card = function (btn) {
  const card = btn.closest(".m2-position-card");
  if (!card) return;
  const body = card.querySelector(".m2-card-body");
  if (!body) return;
  const open = body.style.display === "block";
  body.style.display = open ? "none" : "block";
  btn.textContent = open ? "展開整組" : "收合整組";
};
...
window.collapseAllM2Details = function () {
  document.querySelectorAll(".m2-position-card").forEach((card) => {
    ...
    if (btn) btn.textContent = "展開整組";
  });
};

window.toggleM3Card = function (btn) {
  const card = btn.closest(".m3a-stock-card");
  if (!card) return;
  const body = card.querySelector(".m3-card-body");
  if (!body) return;
  const open = body.style.display === "block";
  body.style.display = open ? "none" : "block";
  btn.textContent = open ? "展開整卡" : "收合整卡";
};
...
window.collapseAllM3A = function () {
  document.querySelectorAll(".m3a-stock-card").forEach((card) => {
    ...
    const btn = card.querySelector(".m3a-detail-btn");
    if (body) body.style.display = "none";
    if (btn) btn.textContent = "展開整卡";
  });
};

window.toggleM3BCard = function (btn) {
  const card = btn.closest(".m3b-card");
  if (!card) return;
  const body = card.querySelector(".m3b-card-body");
  if (!body) return;
  const open = body.style.display === "block";
  body.style.display = open ? "none" : "block";
  btn.textContent = open ? "展開整組" : "收合整組";
...
window.expandAllM3B = function () {
  document.querySelectorAll(".m3b-card").forEach((card) => {
    const body = card.querySelector(".m3b-card-body");
    const btn = card.querySelector(".m3b-detail-btn");
    if (body) body.style.display = "block";
    if (btn) btn.textContent = "收合整組";
  });
};

window.collapseAllM3B = function () {
  document.querySelectorAll(".m3b-card").forEach((card) => {
    const body = card.querySelector(".m3b-card-body");
    const btn = card.querySelector(".m3b-detail-btn");
    if (body) body.style.display = "none";
