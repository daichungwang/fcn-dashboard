(function(){
  ...
  const STRATEGY_TARGETS={'長期穩定現金流':40,'合理投資型':30,'積極單':20,'短期投機單':10};
  ...
  function inputByStrategy(rows){const out={};arr(rows).forEach(r=>{const b=bucket(r);if(STRATEGY_TARGETS[b]!=null)out[b]=(out[b]||0)+amountWan(r);});return out;}
  function evalSignal(rate){if(rate>=95)return{label:'健康',color:'green',color_class:'good',level:'good'};if(rate>=90)return{label:'注意',color:'yellow',color_class:'warn',level:'warn'};return{label:'偏低',color:'red',color_class:'bad',level:'bad'};}
