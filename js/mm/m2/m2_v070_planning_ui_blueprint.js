...
// Dynamic stage caps + dynamic allocation loop for Section C/D/E0
// ============================================================
(function(){
  ...
  function bankRowsFromAmounts(amounts){const targets=bankTargets();return Object.keys(targets).map(bank=>{const used=n(amounts[bank],0);const target=n(targets[bank],0);const gap=Math.max(0,target-used);return {bank,used_wan:used,target_wan:target,gap_wan:floorWan(gap),gap_pct:target>0?gap/target*100:0};}).sort((a,b)=>b.gap_pct-a.gap_pct||(BANK_RULES[a.bank]?.priority||99)-(BANK_RULES[b.bank]?.priority||99));}
  ...
  function chooseBankByGap(stageRemaining, bankWorking){for(const r of bankRowsFromAmounts(bankWorking)){const rule=BANK_RULES[r.bank]||{min_wan:1,max_wan:3};if(r.gap_wan>=rule.min_wan&&stageRemaining>=rule.min_wan)return r;}const f=bankRowsFromAmounts(bankWorking).find(r=>r.bank==='富邦');return stageRemaining>=1&&f&&f.gap_wan>=1?f:null;}
  ...
  function publishHandoff(plan){
    const steps=(plan.allocation_steps||[]).map(s=>({
      step:s.step,stage:s.stage_title,stage_id:s.stage_id,strategy:s.strategy_title,bank:s.bank,source:BANK_SOURCE[s.bank]||'',amount_wan:s.amount_wan,status:s.status,
      before_strategy_gap_pct:s.before_strategy_gap_pct,after_strategy_gap_pct:s.after_strategy_gap_pct,before_bank_gap_pct:s.before_bank_gap_pct,after_bank_gap_pct:s.after_bank_gap_pct,
      rank_rule:'largest_strategy_gap_pct_then_largest_bank_gap_pct_recompute_each_step'
    }));
    const handoff={source:'maturity_cashflow_d',version:'v071_dynamic_allocation_loop',updated_at:new Date().toISOString(),steps};
    window.__M2_TO_MARKET_FCN_HANDOFF__=handoff;
    plan.handoff_to_market_fcn=steps.map(s=>({step:s.step,find:s}));
    return handoff;
  }
  ...
  function buildAllocationPlan(){
    ...
    stagesDef.forEach(stage=>{
      ...
      while(remaining>0){
        ...
        const beforeStrategyRows=rowsFromAmounts(working);
        const beforeBankRows=bankRowsFromAmounts(bankWorking);
        const beforeStrategy=beforeStrategyRows.find(r=>r.strategy===target.strategy)||target;
        const beforeBank=beforeBankRows.find(r=>r.bank===bank)||bankChoice;
        const amt=allocateLot(bank,remaining,target.need_wan,bankChoice.gap_wan);
        ...
        const afterStrategyRows=rowsFromAmounts(working);
        const afterBankRows=bankRowsFromAmounts(bankWorking);
        const afterStrategy=afterStrategyRows.find(r=>r.strategy===target.strategy)||{};
        const afterBank=afterBankRows.find(r=>r.bank===bank)||{};
        const s={
          step:stepNo++,stage_id:stage.stage_id,stage_title:stage.title,operation_type:target.strategy,strategy_title:target.strategy,bank,amount_wan:amt,status:stage.stage_id==='strategic'?'candidate_only':'planned',display_title:`Step ${stepNo-1}｜${target.strategy}｜${bank}｜${wan(amt)}`,
          before_strategy_gap_pct:beforeStrategy.gap_pct,after_strategy_gap_pct:afterStrategy.gap_pct,before_strategy_need_wan:beforeStrategy.need_wan,after_strategy_need_wan:afterStrategy.need_wan,
          before_bank_gap_pct:beforeBank.gap_pct,after_bank_gap_pct:afterBank.gap_pct,before_bank_gap_wan:beforeBank.gap_wan,after_bank_gap_wan:afterBank.gap_wan,remaining_wan:remaining,
          recompute_note:'after this step, strategy gap% and bank gap% are recalculated before next allocation'
        };
        allocation_steps.push(s);stageSteps.push(s);trace.push(s.display_title);
      }
      stages.push({...stage,planned_wan:stage.available_wan-remaining,remaining_wan:remaining,steps:stageSteps,trace,by_strategy:summarizeAmounts(stageSteps,'strategy_title'),by_bank:summarizeAmounts(stageSteps,'bank')});
    });
    const plan={version:'v071_dynamic_allocation_loop',unit:'wan_usd',stage_caps:getStageCaps(),base_policy:{total_plan_base_wan:getPlanBaseWan()},strategy_gap_rows:rowsFromAmounts(getRuntimeAmounts()),bank_gap_rows:bankRowsFromAmounts(getRuntimeBankAmounts()),final_strategy_gap_rows:rowsFromAmounts(working),final_bank_gap_rows:bankRowsFromAmounts(bankWorking),stages,allocation_steps,runtime_context:getCtx()};
    publishHandoff(plan);return plan;
  }

  function renderKpis(){const plan=buildAllocationPlan();window.__M2_ALLOCATION_PLAN_BLUEPRINT__=plan;const caps=getStageCaps();const total=(plan.allocation_steps||[]).reduce((s,x)=>s+n(x.amount_wan),0);const s0=plan.stages[0]||{},s1=plan.stages[1]||{},s2=plan.stages[2]||{};return `<div class="m2v070-kpis"><div class="m2v070-kpi" style="--accent:#0f766e"><label>優先規劃｜確定可用</label><b>${wan(s0.available_wan)}</b><span>${s0.sub}</span>${renderMiniSteps(s0.steps)}</div><div class="m2v070-kpi" style="--accent:#2563eb"><label>短期規劃｜條件式啟用</label><b>${wan(s1.available_wan)}</b><span>${s1.sub}</span>${renderMiniSteps(s1.steps)}</div><div class="m2v070-kpi" style="--accent:#7c3aed"><label>策略佈局｜候補資金</label><b>${wan(s2.available_wan)}</b><span>${s2.sub}</span>${renderMiniSteps(s2.steps)}</div><div class="m2v070-kpi" style="--accent:#f97316"><label>預計池 / 已規劃</label><b>${wan(caps.expectedPool)} / ${wan(total)}</b><span>每一筆 allocation 後即重新計算 strategy gap% / bank gap%。</span></div></div>`;}
