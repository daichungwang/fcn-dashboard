...
const STORAGE_KEY = "fcn_m4_records_v9";
...
function getKIScoreComment(score) {
  if (score === -999) return "KI > 75，僅供參考";
  if (score === 8) return "KI ≤ 55，安全性高，僅供參考";
  if (score === 4) return "KI 55–60，偏安全，僅供參考";
  if (score === 0) return "KI 60–65，標準，僅供參考";
  if (score === -4) return "KI 65–70，偏高，僅供參考";
  if (score === -8) return "KI 70–75，風險偏高，僅供參考";
  return "僅供參考";
}

function getStrikeScoreComment(score) {
  if (score === -999) return "Strike > 80，僅供參考";
  if (score === 10) return "Strike ≤ 60，安全，僅供參考";
  if (score === 5) return "Strike 60–65，合理，僅供參考";
  if (score === -1) return "Strike 65–67，略高，僅供參考";
  if (score === -3) return "Strike 67–70，偏高，僅供參考";
  if (score === -5) return "Strike 70–75，風險提高，僅供參考";
  if (score === -10) return "Strike 75–80，偏危險，僅供參考";
  return "僅供參考";
}
...
export function evaluateDeal(deal = {}) {
  ...
  return {
    ...
    l2: {
      ...
      ki_score: kiScore,
      ki_comment: getKIScoreComment(kiScore),

      strike_value: toNumber(deal.strike),
      strike_score: strikeScore,
      strike_comment: getStrikeScoreComment(strikeScore),
