...
function getTopNews(data) {
  ...
  return all.filter((n) => String(n.level || "").toLowerCase() === "high").slice(0, 3);
}

function renderMarketTable(marketData) {
  const rows = Array.isArray(marketData) ? marketData : Object.entries(marketData || {}).map(([name, value]) => ({
    name,
...
function renderChoiceButtons(key, currentDirection) {
  ...
  return `
    <div class="news-choice-row" data-news-key="${key}">
      <button type="button" class="news-choice-btn ${current === normalizeDirection("ai") ? "active" : current === normalizeDirection(currentDirection) ? "active-ai" : ""}" onclick="setNewsDirection('${key}', 'ai', this)">採用AI</button>
      <button type="button" class="news-choice-btn ${current === "positive" ? "active" : ""}" onclick="setNewsDirection('${key}', 'positive', this)">正向</button>
      <button type="button" class="news-choice-btn ${current === "neutral" ? "active" : ""}" onclick="setNewsDirection('${key}', 'neutral', this)">中性</button>
      <button type="button" class="news-choice-btn ${current === "negative" ? "active" : ""}" onclick="setNewsDirection('${key}', 'negative', this)">負向</button>
    </div>
  `;
}
...
function renderNewsCard(news, key, expanded = false) {
  ...
  return `
    <div class="news-card">
      ...
      ${renderChoiceButtons(key, window.__module1Overrides?.[key] || news.ai_direction || "neutral")}
